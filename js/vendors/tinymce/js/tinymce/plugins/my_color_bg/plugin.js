...
tinymce.PluginManager.add('my_color_bg', function (editor) {
    var lastPanel, previousPanel, opacitySlider, lastColor = 'rgba(255,255,255,1)';
    ...
    function renderColorPicker(sideBlock, mainBlock, blackBlock) {
        ...
        function getColorCellHtml(color, title) {
            ...
            return (
            '<td class="mce-gridBg-cell' + (isNoColor ? ' mce-colorbtn-trans' : '') + '">' +
            '<div id="' + 'my_color_bg' + '-' + (count++) + '"' +
        ...
        html += '<div class="opacitySliderBg clearfix"><span>Opacity</span><input type="text" min="0" max="100" id="opacitySliderTextBg" value="100"><div id="opacitySliderBg"></div></div>';

        return html;
    }

    function renderColorMenuPanel(sideBlock, mainBlock, blackBlock) {
        return '<div id="colorMenuPanelBg" class="colorMenuPanelBg" style="display: none">' + renderColorPicker(sideBlock, mainBlock, blackBlock) + '</div>';
    }

    function mceGridCellListener() {
        $('.mce-gridBg-cell').click(function (e) {
    ...
    function opacitySliderTextListener() {
        $('#opacitySliderTextBg').change(function (e) {
            if (!e.target.value)
                return -1;
            if (e.target.value)
    ...
    function applyChooseColor(value) {
        var $headerColor = $('.mce-colorbutton-gridBg-header-color'),
            $colorPicker__color = $('.colorPickerBg__color'),
            $allCells = $('.mce-gridBg-cell'),
            $opacitySlider = $('#opacitySliderBg'),
            $opacitySliderText = $('#opacitySliderTextBg'),
            rgbValue, cellRgb;
